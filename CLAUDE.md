...
| File | Covers |
| --- | --- |
| [arrangement.test.js](test/arrangement.test.js) | 120 bars, 4:00, contiguous sections, chord shapes, `mtof`, `frand`, and the intensity curve: shared endpoints, no step at a boundary, phrase alignment, lead-vs-harmony |
...
- [audio/arrangement.js](src/audio/arrangement.js) — the score as data, plus the pure arithmetic over it. `SECTIONS` (ten named sections × bar counts = 120 bars = 4:00 at 120 BPM written in half-time), `PROG` (i–VI–III–VII in D minor), `ARP`, `LEAD`, and the `BPM`/`SPB`/`STEP`/`BAR` maths. **Edit this file to change the shape of the piece; edit engine.js to change how it sounds.**
  - **Dynamics belong to the piece, not to the section.** Each section declares `i0`/`i1` — the intensity it comes in at and leaves at — and `i1` of one *is* `i0` of the next. `intensityAt(step)` interpolates, and every level the sequencer scales hangs off that one number. This replaced a per-section `p = 0..1` that made all ten sections crescendo from their own floor: measured, that cost between 25% and 77% of the scheduled energy at each of the nine section changes, and it sounded like the piece restarting ten times. Sections still *fall* — FALLING and HORIZON are meant to — they just fall from where the previous one left off. [test/arrangement.test.js](test/arrangement.test.js) asserts the shared endpoints, so one edited number cannot quietly reopen a seam.
  - `PHRASE` is the four-bar grid. Pads, the tick figure and the melody group on the **absolute** bar, never the bar within the section, so their phrasing does not re-phase where sections meet — which is only sound because no section starts mid-phrase, and a test holds the bar counts to that.
- [audio/engine.js](src/audio/engine.js) — the `Audio` singleton: Web Audio graph, sequencer, analyser.
  ...
  - **What a section change is allowed to change is texture, not level.** A drop arrives by bringing the organ ranks, the choir stack and the tick an octave up; a break departs by taking them away. Levels come from `intensityAt` and run straight through the downbeat. `p` (progress within the section) survives only where something genuinely stages *inside* a section — voices entering during the intro, the timpani pulse subdividing through a build, the outro thinning out. **If you find yourself scaling a level by `p`, that is the bug this design exists to prevent.**
  - **Sustained voices crossfade rather than retrigger.** `pedal`, `strings`, `choir` and `organ` hold to their full `dur` and release over exactly their attack time, so consecutive blocks — which sit exactly `dur` apart on the absolute-bar grid — are mirror images that sum to a constant. They used to release *before* `dur`, which put a dip under every chord change and every section boundary. The pedal's sub-octave is pointedly excluded: two sines an octave down at neighbouring roots overlapping is how you get a cancellation notch in the one octave with no headroom to spare.
